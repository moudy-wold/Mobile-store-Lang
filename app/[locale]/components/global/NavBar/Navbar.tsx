...
function Navbar({ locale }: Props) {
  ...
  const { infoData, card_System, repair_Service_System } = useSelector(
      ...
      >
        {!path.includes("notfound") && !path.includes("update-plane") && (
          <>
            {/* Start Burger Menu */}
            <div
              className={`lg:hidden grid grid-cols-[78%_20%]  items-center `}
            >
              <div className=" flex">
                <div className="mx-5">
                  <GiHamburgerMenu
                ...
                {isAdmin && (
                  <>
                    {!adminPage && (
                      <div className="">
                        <Link href="/admin">
                          <GrUserAdmin className="text-3xl cursor-pointer " />
                        </Link>
                      </div>
                    )}
                  </>
            ...
            <div className="hidden lg:flex items-center justify-between">
              ...
              <div className="flex items-center ">
                ...
                <Link href="/" className="mr-2">
                  <Image
                    src={
                      infoData?.data?.logo != null ? infoData?.data?.logo : "/"
                    }
                    height={154}
                    width={154}
                    alt="Logoss"
