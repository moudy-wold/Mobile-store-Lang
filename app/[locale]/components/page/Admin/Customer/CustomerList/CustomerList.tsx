  ...
  const columns: ColumnsType<any> = [
    {
      ...
      render: (_, record) => ( <a href={`/user-profile/${record._id}`}>{record.name}</a>),
    },
    {
      title: "رقم الهاتف",
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <a href={`/admin/customer/edit/${record._id}`}><CiEdit /></a>
          ...
          <a href={`/admin/message`}><IoChatbubblesOutline onClick={() => { setCustomerId(record._id) }} className="text-xl" /></a>
        ...
        <span className="hover:text-[#006496] cursor-pointer hover:scale-110 transition-all duration-200" onClick={() => { setCustomerId(record._id); setOpenChangePassword(true) ; setCusotmerPhoneNumber(record.phoneNumber)}} ><GiFlowerStar /></span>
  ...
  const serviceColumns: ColumnsType<any> = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <select
            onChange={(e) => { onFinish(record._id, e.target.value); setStatus(e.target.value); setServiceId(record._id); }}
            style={{ width: "100%" }}
